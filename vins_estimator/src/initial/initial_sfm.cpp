...
/**
 * @brief 根据上一帧的位姿通过opencv-pnp求解当前帧的位姿
 *
								...
								vector<SFMFeature> &sfm_f)
{
	...
	{
		ROS_WARN("unstable features tracking, please slowly move you device!\n");
		// 实在是太少了,进行一个gg
		if (int(pts_2_vector.size()) < 10)
			return false;
	}

	// OPENCV求解PNP ,然后数据格式转换
						  ...
						  vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
	...
	// Step 2 考虑有些特征点不能被最后一帧看到，对第l帧与第l+1到frame_num -2的每一帧再进行三角化
	...
	//              l-2              l-2 ----- l
	// PNP求解从第l-1到第0帧的每一帧与第l帧之间的变换矩阵，并进行三角化
	for (int i = l - 1; i >= 0; i--)
	{
		// solve pnp
		Matrix3d R_initial = c_Rotation[i + 1];
