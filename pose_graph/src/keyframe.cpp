...
void KeyFrame::computeWindowBRIEFPoint()
{
	...
	// 计算VIO节点提取的特征点的描述子
	// 重载了operator() 调用DBOW库来计算
	// 保存到window_brief_descriptors里
 ...
 * @param[in] matched_3d 当前帧3d地图点VIO节点下
 * @param[out] status
 * @param[out] PnP_T_old
 * @param[out] PnP_R_old
 */
...
bool KeyFrame::findConnection(KeyFrame *old_kf)
{
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		...
		// 合理范围之内才认为有效 如果查了太多可信度不高
		// 没有算金子塔,如果两帧的平移太大,描述子不可信
		if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0)
		{
			has_loop = true;			// 至此才认为找到了回环
			loop_index = old_kf->index; // 确定回环帧的idx
			//以及两帧之间相对位姿
